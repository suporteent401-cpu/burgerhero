...
export const PlanFormModal: React.FC<PlanFormModalProps> = ({ isOpen, onClose, onSubmit, editingPlan }) => {
  const { register, handleSubmit, control, reset, watch } = useForm<Plan>({
    defaultValues: {
      name: '',
      priceCents: 0,
      description: '',
  ...
  const { fields, append, remove } = useFieldArray({
    control,
    name: "benefits" as any, // react-hook-form typing issue with string[]
  });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={editingPlan ? 'Editar Plano' : 'Novo Plano'}>
      <form onSubmit={handleSubmit(handleFormSubmit)} className="flex flex-col md:flex-row gap-8">
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => append({ value: '' })}>
                <Plus size={16} className="mr-2" /> Adicionar Benefício
              </Button>
            </div>
          </div>
        ...
        <div className="w-full md:w-64 flex-shrink-0">
          <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Preview do Card</h4>
      ...
      <div className="flex justify-end gap-2 pt-6 border-t border-slate-100 mt-6">
        ...
        <Button type="button" onClick={handleSubmit(handleFormSubmit)}>{editingPlan ? 'Salvar Alterações' : 'Criar Plano'}</Button>
      </div>
    </Modal>
  );
};