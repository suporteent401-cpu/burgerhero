    ...
    <div className="space-y-6 pb-10">
      ...
      {hasUnsavedChanges && (
        <div className="sticky top-[70px] z-30 bg-amber-500 p-3 rounded-2xl shadow-lg flex items-center justify-between text-white animate-in slide-in-from-top-4">
          <div className="flex items-center gap-2"><AlertTriangle size={16} /> <span className="text-xs font-bold">Alterações pendentes</span></div>
          <div className="flex gap-2">
            <Button size="sm" variant="ghost" onClick={handleCancelChanges} className="text-white hover:bg-white/20 h-8"><RotateCcw size={14} /></Button>
            <Button size="sm" onClick={handleSaveSettings} isLoading={isSaving} className="h-8 bg-white text-amber-700 shadow-sm"><Save size={14} className="mr-1.5" /> Salvar</Button>
          </div>
        </div>
      )}
        ...
        <div className="mt-4 flex flex-col items-center">
          {isEditingName ? (
             <div className="flex items-center gap-2">
                <input className="bg-transparent border-b-2 border-hero-primary text-xl font-black text-center dark:text-white" value={editName} onChange={e => setEditName(e.target.value)} autoFocus />
          ...
          <div className="bg-slate-50 dark:bg-slate-800 px-4 py-1.5 rounded-full flex items-center gap-2">
            ...
            <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">ID: {user?.customerCode}</p>
          </div>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="flex justify-between items-center">
          <div className="flex items-center gap-2 font-black text-xs uppercase tracking-widest text-slate-400"><CreditCard size={14} /> Editor do Cartão</div>
          {saveSuccess && <span className="text-xs font-bold text-green-600 flex items-center gap-1"><CheckCircle2 size={12} /> Sincronizado</span>}
        </CardHeader>
        <CardBody className="p-6 space-y-6">
          <div className="grid grid-cols-3 gap-3">
            {availableTemplates.map(t => (
              <button key={t.id} onClick={() => updateDraft('templateId', t.id)} className={`relative rounded-xl overflow-hidden aspect-[1.586/1] ${draft.templateId === t.id ? 'ring-4 ring-hero-primary scale-[1.02]' : 'ring-1 ring-slate-200'}`}>
                <img src={t.imageUrl} alt={t.name} className="w-full h-full object-cover" />
                {draft.templateId === t.id && <div className="absolute inset-0 bg-hero-primary/20 flex items-center justify-center"><CheckCircle2 className="text-white" /></div>}
              </button>
            ))}
          </div>
          <div className="space-y-4">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {FONT_OPTIONS.map(f => (
                  <button key={f.name} onClick={() => updateDraft('fontFamily', f.value)} style={{ fontFamily: f.value }} className={`px-3 py-1.5 rounded-lg text-sm border ${draft.fontFamily === f.value ? 'bg-hero-primary text-white' : 'bg-white'}`}>{f.name}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {COLOR_OPTIONS.map(c => (
                  <button key={c.name} onClick={() => updateDraft('fontColor', c.value)} className={`w-8 h-8 rounded-full border-2 ${draft.fontColor === c.value ? 'ring-2 ring-hero-primary' : ''}`} style={{ backgroundColor: c.value }} />
                ))}
              </div>
            </div>
          </div>
          <div className="pt-4 flex flex-col items-center">
             <div className="flex justify-between w-full mb-2">
               ...
               <Button onClick={handleExportCard} variant="ghost" size="sm"><Download size={14} /></Button>
             </div>
             <HeroCard 
               ref={cardRef} 
               user={user} 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader><div className="flex items-center gap-2 font-black text-xs uppercase text-slate-400"><Palette size={14} /> Tema do App</div></CardHeader>
          <CardBody className="p-4 grid grid-cols-5 gap-2">
            {themes.map(t => (
              <button key={t.name} onClick={() => updateDraft('heroTheme', t.name)} className={`w-8 h-8 rounded-full border-2 ${draft.heroTheme === t.name ? 'border-hero-primary scale-110 shadow-lg' : 'border-transparent'}`} style={{ backgroundColor: t.color }} />
            ))}
          </CardBody>
        </Card>
        <Card>
          <CardHeader><div className="flex items-center gap-2 font-black text-xs uppercase text-slate-400"><Monitor size={14} /> Modo</div></CardHeader>
          <CardBody className="p-4 flex gap-2">
              <button onClick={() => updateDraft('mode', 'light')} className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border-2 ${draft.mode === 'light' ? 'border-hero-primary text-hero-primary' : ''}`}><Sun size={18} /> <span className="text-xs font-bold">CLARO</span></button>
              <button onClick={() => updateDraft('mode', 'dark')} className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border-2 ${draft.mode === 'dark' ? 'border-hero-primary text-hero-primary' : ''}`}><Moon size={18} /> <span className="text-xs font-bold">ESCURO</span></button>
          </CardBody>
        </Card>
      </div>
