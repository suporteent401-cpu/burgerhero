...
const Home: React.FC = () => {
  ...
  const { getSelectedTemplate, selectedFont, selectedColor, selectedFontSize } = useCardStore();
      ...
      <div className="flex justify-between items-center px-1">
        ...
        <Link to="/app/profile" className="relative">
          <div className="w-16 h-16 rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden border-2 border-white dark:border-slate-700 shadow-md">
            <img src={user?.avatarUrl || `https://picsum.photos/seed/${user?.id || 'default'}/100`} alt="Avatar" className="w-full h-full object-cover" />
          </div>
        </Link>
      </div>
             ...
             <div className="divide-y divide-slate-50 dark:divide-slate-800">
                <div className="p-4 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200">Hambúrguer do Mês</p>
                    <p className="text-xs text-slate-400 font-medium">
                      {benefit?.burgerRedeemed ? 'Resgatado' : 'Pendente de resgate'}
                    </p>
                  </div>
